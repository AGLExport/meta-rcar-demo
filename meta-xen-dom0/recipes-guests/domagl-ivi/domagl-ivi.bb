...
SRC_URI = "\
    file://domagl-ivi-virtio.cfg \
    file://domagl-ivi.service \
"

FILES:${PN} = " \
    ${sysconfdir}/xen/domagl-ivi.cfg \
    ${libdir}/xen/boot/domu.dtb \
    ${libdir}/xen/boot/linux-domu \
    ${systemd_unitdir}/system/domagl-ivi.service \
"

SYSTEMD_SERVICE:${PN} = "domagl-ivi.service"
SYSTEMD_AUTO_ENABLE:${PN} = "${XT_AUTOSTART_DOMAGL_IVI}"
...
do_install() {
    ...
    install -m 0644 ${WORKDIR}/domagl-ivi-virtio.cfg ${D}${sysconfdir}/xen/domagl-ivi.cfg
    ...
    install -m 0644 ${WORKDIR}/domagl-ivi.service ${D}${systemd_unitdir}/system/
}
...
FILES:${PN} += " \
    ${sysconfdir}/systemd/system/domagl-ivi.service.d/virtio-env.conf \
    ${sysconfdir}/systemd/system/domagl-ivi.service.d/virtio-env-weston.conf \
"

do_install:append() {
    install -d ${D}${sysconfdir}/systemd/system/domagl-ivi.service.d
    install -m 0644 ${WORKDIR}/virtio-env.conf ${D}${sysconfdir}/systemd/system/domagl-ivi.service.d
